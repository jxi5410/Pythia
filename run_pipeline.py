...
def main():
    ...
    resolved_db = Path(args.db).expanduser()
    if not resolved_db.is_absolute():
        resolved_db = (Path(__file__).resolve().parent / resolved_db).resolve()
    args.db = str(resolved_db)
    os.environ["PYTHIA_DB_PATH"] = args.db
    ...
    # Do not inject hardcoded credentials at runtime.
    if not os.environ.get("NEWSAPI_KEY"):
        logging.warning("NEWSAPI_KEY is not set; attribution quality may be reduced.")
