...
class PaperTrading:
    ...
    def create_trade_from_signal(self, signal: Dict) -> Optional[int]:
        ...
        position_size *= self._correlation_penalty(signal)
        if position_size <= 0:
            return None
    ...
    def _can_open_position(self, signal: Dict) -> bool:
        ...
        # Block new trades when correlated exposure is already concentrated.
        if self._correlation_penalty(signal) <= 0:
            return False
    ...
    def _correlation_penalty(self, signal: Dict) -> float:
        """
        Reduce sizing when a new trade overlaps with existing open positions.

        Uses statistical market correlations when available and falls back
        to category overlap as a pragmatic concentration proxy.
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            rows = conn.execute(
                """
                SELECT pt.market_id, COALESCE(m.category, '')
        ...
        open_market_ids = {str(row[0]) for row in rows if row[0]}
        overlapping_categories = sum(1 for row in rows if category and row[1] == category)

        correlated_hits = 0
        if market_id and open_market_ids:
            try:
                pairs = self.analytics_db.get_correlations(market_id=market_id)
            except Exception:
                pairs = []
            for pair in pairs:
                other_market_id = (
                    pair.get("market_id_b") if pair.get("market_id_a") == market_id
                    else pair.get("market_id_a")
                )
                rho = abs(float(pair.get("spearman_rho", 0.0) or 0.0))
                if other_market_id in open_market_ids and rho >= 0.5:
                    correlated_hits += 1

        penalty = 1.0 - 0.25 * overlapping_categories - 0.20 * correlated_hits
        if overlapping_categories >= 2 or correlated_hits >= 2:
            penalty -= 0.15
        floor = max(0.0, 1.0 - float(self.config.MAX_CORRELATED_EXPOSURE_PENALTY))
        return 0.0 if penalty <= 0 else max(floor, penalty)
