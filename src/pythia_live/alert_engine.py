"""
Pythia Alert Engine — Configurable alert rules with cooldown and Telegram delivery.
...
class AlertEngine:
    ...
    Usage::
        ...
        engine = AlertEngine(bot_token="...", chat_id="...")
    ...
    def __init__(
        ...
        self.bot_token = bot_token or os.environ.get("PYTHIA_TELEGRAM_TOKEN", "")
        self.chat_id = chat_id or os.environ.get("PYTHIA_TELEGRAM_CHAT_ID", "")
    ...
    def _fire(self, rule: AlertRule, key: str, message: str) -> Optional[FiredAlert]:
        ...
        for channel in rule.channels:
            if channel == "telegram":
                success = _send_telegram(self.bot_token, self.chat_id, message)
                if success:
                    delivered_to.append("telegram")
                else:
                    delivered_to.append("telegram:failed")
