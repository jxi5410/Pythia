...
def _compute_realized_trade_metrics(
    db: PythiaDB,
    days: int,
) -> Dict[str, float]:
    """Compute risk-adjusted metrics from closed paper trades in the lookback window."""
    ...
    returns = [float(row[0]) for row in rows if row and row[0] is not None]
    if not returns:
        return {
            "realized_trade_count": 0,
            "avg_realized_return": 0.0,
    ...
    variance = sum((r - avg_return) ** 2 for r in returns) / max(1, len(returns) - 1)
    std_dev = variance ** 0.5
    sharpe_ratio = (avg_return / std_dev) * (len(returns) ** 0.5) if std_dev > 0 else 0.0
    ...
    win_loss_ratio = avg_win / avg_loss if avg_loss > 0 else float(len(wins) > 0)

    return {
        "realized_trade_count": len(returns),
...
def get_track_record(
    ...
    # --- 8. Realized trade metrics ---
    realized_metrics = _compute_realized_trade_metrics(db, days)
    ...
    return TrackRecord(
        ...
        avg_realized_return=realized_metrics["avg_realized_return"],
        sharpe_ratio=realized_metrics["sharpe_ratio"],
        max_drawdown=realized_metrics["max_drawdown"],
        win_loss_ratio=realized_metrics["win_loss_ratio"],
...
def format_track_record(record: TrackRecord) -> str:
    ...
    lines.append(f"Closed trades: {record.realized_trade_count}")
    lines.append(f"Avg realized return: {record.avg_realized_return:.2%}")
    lines.append(f"Sharpe ratio: {record.sharpe_ratio:.2f}")
    lines.append(f"Max drawdown: {record.max_drawdown:.2%}")
    lines.append(f"Win/loss ratio: {record.win_loss_ratio:.2f}")
...
def format_track_record_for_pdf(record: TrackRecord) -> Dict:
    ...
    return {
        ...
        "summary": {
            ...
            "realized_trade_count": record.realized_trade_count,
            "avg_realized_return": record.avg_realized_return,
            "sharpe_ratio": record.sharpe_ratio,
            "max_drawdown": record.max_drawdown,
            "win_loss_ratio": record.win_loss_ratio,
        },
        "thresholds": [
            {
