...
class ProbabilityModel:
    ...
    def fit(self, price_history: List[float]) -> None:
        ...
        if not np.isfinite(variance) or variance <= _MIN_VARIANCE or np.allclose(prices, prices[0], atol=1e-4):
            self._fit_fallback(mean_price, variance, prices.size)
            return
    ...
    def _fit_fallback(self, mean_price: float, variance: float, sample_size: int) -> None:
        """Use a stable method-of-moments fallback for low-variance histories."""
        m = float(np.clip(mean_price, _EPS, 1.0 - _EPS))
        max_v = max(_EPS, m * (1.0 - m) - _EPS)
        if not np.isfinite(variance) or variance <= _MIN_VARIANCE:
            common = _FALLBACK_CONCENTRATION
        else:
            v = min(max(float(variance), _EPS), max_v)
            common = max(_EPS, m * (1.0 - m) / v - 1.0)

        self.alpha = max(_EPS, m * common)
        self.beta_param = max(_EPS, (1.0 - m) * common)
        self.n_observations = int(sample_size)
